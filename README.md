...
Using custom vLLM endpoint outside AWS can be done as follows:

1. Build FMBench normally as per [instructions](https://aws-samples.github.io/foundation-model-benchmarking-tool/build.html).
1. Use existing configs (e.g. [Llama3.1-70b](fmbench/configs/llama3.1/70b/config-llama3-1-70b-vllm-rocm.yml)) or create your own. Set `CONFIG_FILE` point to your config file:
   ```{.bash}
   export CONFIG_FILE=fmbench/configs/llama3.1/70b/config-llama3-1-70b-vllm-rocm.yml
    ```
1. Start your vLLM endpoint to serve the model used in the config.
1. Setup temp folder for FMBench:
   ```{.bash}
   ...
   ``` 
   Make sure `$TMP_DIR/fmbench-read/configs/pricing.yml` contains pricing info for your endpoint type.
1. Run FMBench in local mode through CLI command as:
   ```{.bash}
   fmbench --config-file $CONFIG_FILE --local-mode yes --write-bucket placeholder --tmp-dir $TMP_DIR > fmbench.log 2>&1
   ```
